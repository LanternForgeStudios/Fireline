...
- [x] Mobile touch controls (aim pads, zoom button) visually shifting/moving off-screen while
      camera zoom was active — fixed (2026-09-05) by wrapping each pad's ring/knob/label and the
      zoom button's circle/label in a `Phaser.GameObjects.Container`, then counter-scaling/
      -positioning each container every frame (`syncTouchUiForZoom()`) to cancel out the camera's
      zoom transform, so they render at a fixed screen position/size regardless of zoom level.
      Cheaper than a second `.ignore()`-based UI camera, which would've required registering every
      dynamically-created world object in `CombatScene.ts` against its ignore list. See
      PROGRESS.md entry 48.
